...
from op import down_sample, down_sample_group, class_predictor, box_predictor, flatten_prediction, concat_predictions, \
    stem, dense_block, transition_layer
...
def mobile_body():
    out = nn.HybridSequential()
    inchannels = 3
    for nfiters in [16, 32, 64]:
        out.add(down_sample_group(inchannels, nfiters))
        inchannels = nfiters
    return out


def mobile_net_model(num_anchors, num_classes):
    body_net = mobile_body()
    downsamples = nn.Sequential()
    class_preds = nn.Sequential()
    box_preds = nn.Sequential()

    downsamples.add(down_sample_group(64, 128))
    downsamples.add(down_sample_group(128, 128))
    downsamples.add(down_sample_group(128, 128))
    for scale in range(5):
        class_preds.add(class_predictor(num_anchors, num_classes))
        box_preds.add(box_predictor(num_anchors))
...
def mobile_net_forward(x, body, downsamples, class_preds, box_preds, sizes, ratios):
    x = body(x)
    default_anchors = []
    predicted_boxes = []
    predicted_classes = []
...
class MobileNet(gluon.Block):
    def __init__(self, num_classes, **kwargs):
        super(MobileNet, self).__init__(**kwargs)
        self.anchor_sizes = [[.08],[.2], [.37], [.54], [.71]]
        self.anchor_ratios = [[1.]] * 5
        self.num_classes = num_classes
        with self.name_scope():
            self.body, self.downsamples, self.class_preds, self.box_preds = mobile_net_model(1, num_classes)

    def forward(self, x):
        default_anchors, predicted_boxes, predicted_classes = mobile_net_forward(x, self.body, self.downsamples, \
                                                                                 self.class_preds, self.box_preds,
                                                                                 self.anchor_sizes, self.anchor_ratios)
